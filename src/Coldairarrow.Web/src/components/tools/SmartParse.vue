...
<script>
import pcassCode from 'vue-smart-parse/src/lib/json/pcas-code.json'
import zipCode from 'vue-smart-parse/src/lib/json/zip-code.json'
...
function smart (event) {
  ...
  copyaddress.forEach((res, index) => {
    if (res) {
      ...
      const addressObj = smatrAddress(res)
...
function smatrAddress (event) {
  ...
  address = handleMunicipality(address) // 特殊城市处理
  ...
  // 邮编匹配扩展
  if (smartObj.zipCode === undefined) {
    const zCode = address.match(
      /^[0-9]{6}$/g
    )
    if (zCode) {
      smartObj.zCode = zCode[0]
      address = address.replace(zCode[0], '')
    }
  }
        ...
        if (
          ...
          el.children.forEach(element => {
            if (element.name === smartObj.city) {
              if (smartObj.city === '东莞市' ||
                  smartObj.city === '嘉峪关市' ||
                  smartObj.city === '三沙市' ||
                  smartObj.city === '儋州市' ||
                  smartObj.city === '中山市'
              ) {
                smartObj.county = smartObj.city
              }
...
/// 直辖市处理
function handleMunicipality (address) {
